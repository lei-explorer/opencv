...
def calibrate_single(imgNums, CheckerboardSize, Nx_cor, Ny_cor, saveFile=False, saveImages=False):
    ...
    while (True):
        ...
        if cv2.waitKey(1) & 0xFF == ord(' '):#在等待的1s之内按下了空格键则执行
            ...
            if ok:  # 如果找到，添加目标点，图像点
                ...
                count += 1
                if saveImages:
                    cv2.imwrite('F:/实验室/B超机器人控制部分/USB摄像头程序/imgs/' + str(count) + '.jpg', frame)
    ...
    if saveFile:
        ...
        np.savez("F:/实验室/B超机器人控制部分/USB摄像头程序/calibrate.npz", mtx=mtx, dist=dist, K=K, D=D)
...
if __name__ == '__main__':
    ...
    mtx,dist,K,D = calibrate_single(30,27,8,6)
    print('mtx=',mtx)
    print('dist=',dist)
    print('K=',K)
    print(type(K))
